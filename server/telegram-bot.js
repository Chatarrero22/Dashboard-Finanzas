...
/** Si hay presupuesto para esa categoria, avisa como viene. */
function avisoPresupuesto(userId, categoria) {
  ...
  var restante = b.monthly_limit - usado;

  if (pct >= 100) {
    return '\n\n🔴 Te pasaste del presupuesto de ' + categoria + ' por ' + money(Math.abs(restante)) +
           '\n(' + money(usado) + ' de ' + money(b.monthly_limit) + ')';
  }
  if (pct >= 80) {
    return '\n\n🟡 Ojo: llevás el ' + Math.round(pct) + '% de ' + categoria +
           '\nTe quedan ' + money(restante) + ' hasta fin de mes';
  }
  return '\n\nTe quedan ' + money(restante) + ' de ' + categoria + ' este mes';
}

function confirmation(userId, tx, itemCount) {
  var msg = (tx.amount > 0 ? '✅ Ingreso guardado' : '✅ Gasto guardado') + '\n\n' +
    tx.description + '\n' +
    money(tx.amount) + '\n' +
    tx.category;
  if (itemCount) msg += '\n' + itemCount + ' productos detallados';
  if (tx.amount < 0) msg += avisoPresupuesto(userId, tx.category);
  return msg;
}

var PEDIR_VINCULO =
  '👋 Hola. Todavía no sé quién sos.\n\n' +
  'Entrá a la app, andá a "Ajustes" y pedí tu código de Telegram. Después escribime:\n\n' +
  '   /vincular 123456\n\n' +
  '(con el número que te dio la app)';
...
function start(config) {
  ...
  bot.onText(/^\/vincular\s*(\d+)?/, function (msg, match) {
    ...
    if (!user) {
      return bot.sendMessage(msg.chat.id, '❌ Ese código no sirve o ya fue usado. Pedí uno nuevo desde la app.');
    }
    bot.sendMessage(msg.chat.id,
      '✅ Listo, ' + user.displayName + '. Ya podés cargar gastos.\n\n' +
      'Probá escribiéndome:  Disco 15400'
    );
  });
  ...
  bot.onText(/^\/(start|help|ayuda)/, function (msg) {
    ...
    bot.sendMessage(msg.chat.id,
      ...
      'Para anotar un gasto, escribime el lugar y el monto. Entiendo como hablás:\n' +
      '   Disco 15400\n' +
      '   gasté 5 lucas en el súper\n' +
      '   un palo y medio el alquiler\n\n' +
      'Si es plata que entra, poné un + adelante:\n' +
      '   +250 lucas sueldo\n\n' +
      '📸 También podés mandarme la foto de un ticket y lo cargo con el detalle.\n\n' +
      'Comandos: /resumen  /metas  /fijos  /ultimos  /borrar'
    );
  });
  ...
  bot.onText(/^\/resumen/, function (msg) {
    ...
    bot.sendMessage(msg.chat.id,
      '📊 Este mes\n\n' +
      'Entró: ' + money(t.income) + '\n' +
      'Gastaste: ' + money(t.expense) + '\n' +
      'Queda: ' + money(t.income - t.expense) + '\n\n' +
      (lines.length ? 'En qué se fue:\n' + lines.join('\n') : 'Todavía no hay gastos este mes.')
    );
  });
  ...
  bot.onText(/^\/ultimos/, function (msg) {
    ...
    if (rows.length === 0) return bot.sendMessage(msg.chat.id, 'Todavía no hay movimientos.');
  ...
  bot.onText(/^\/borrar\s*(\d+)?/, function (msg, match) {
    ...
    bot.sendMessage(msg.chat.id, '🗑️ Borré: ' + row.description + ' ' + money(row.amount));
  });
  ...
  bot.onText(/^\/metas/, function (msg) {
    ...
    if (metas.length === 0) {
      return bot.sendMessage(msg.chat.id, 'No tenés metas de ahorro todavía. Se cargan desde la app.');
    }
  ...
  bot.onText(/^\/fijos/, function (msg) {
    ...
    if (proximas.length === 0) {
      return bot.sendMessage(msg.chat.id, 'No se viene ningún gasto fijo en los próximos 7 días.');
    }
  ...
  bot.on('photo', async function (msg) {
    ...
    bot.sendMessage(msg.chat.id, '📸 Leyendo el ticket, dame unos segundos...');
    try {
  ...
  bot.on('message', async function (msg) {
    ...
    if (!parsed) {
      return bot.sendMessage(msg.chat.id, 'No encontré el monto 🤔\nProbá así: Disco 15400');
    }
    ...
    try {
      ...
      bot.sendMessage(msg.chat.id, 'No lo pude guardar: ' + err.message);
    }
  });
